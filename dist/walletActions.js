...
export const createWalletActions = (provider, config) => {
    ...
    // Get logger instance
    const logger = getLogger(typeof window !== 'undefined' && window.web3DebugEnabled === true);
    ...
    const actions = {
        ...
        requestAccounts: async () => {
            ...
            try {
                debugWalletAction('Calling safeProviderRequest for eth_requestAccounts', {
                    providerInfo: {
                ...
                try {
                    ...
                    debugWalletAction('safeProviderRequest failed, trying direct provider.request...', {
                        error: primaryError.message,
                        code: primaryError.code,
                    });
                    // Fallback: direct provider.request call for MetaMask evmAsk.js issues
                    try {
                        debugWalletAction('Attempting direct provider.request fallback...', {
                            fallbackReason: 'safeProviderRequest failed, likely evmAsk.js internal error',
                        });
                        ...
                        try {
                            ...
                            debugWalletAction('Checking error type for MetaMask selectExtension issue', {
                                ...
                                isTransportError: err.message?.includes('send') ||
                                    err.stack?.includes('#e.send'),
                                isStrictSelectExtension: err.message?.includes('selectExtension') &&
                                    err.message?.includes('Unexpected'),
                            });
                            // Check for MetaMask wallet_selectEthereumProvider dialog failure
                            if ((err.message?.includes('selectExtension') ||
                                ...
                                provider.isMetaMask) {
                                debugWalletAction('Detected MetaMask wallet selection dialog failure', {
                                    error: err.message,
                                    isWalletSelection: err.stack?.includes('wallet_selectEthereumProvider'),
                        ...
                        if (Array.isArray(result) && result.length > 0) {
                            accounts = result;
                            debugWalletAction('Direct provider.request succeeded - accounts found', {
                        ...
                        debugWalletAction('Both safeProviderRequest and direct request failed', {
                            primaryError: {
                            ...
                            fallbackError: {
                                ...
                                isEvmAskError: fallbackError.message?.includes('evmAsk') || false,
                ...
                debugWalletAction('safeProviderRequest failed - Detailed Error Analysis', {
        ...
        signMessage: async (message) => {
            try {
                ...
                const signature = await safeProviderRequest(provider, 'personal_sign', [messageToSign, from], config?.requestTimeout || 60000 // Increase timeout for user interaction
                );
        ...
        signTypedData: async (typedData) => {
            try {
                ...
                const signature = await safeProviderRequest(provider, 'eth_signTypedData_v4', [from, typedDataString], config?.requestTimeout || 60000 // Increase timeout for user interaction
                );
        ...
        switchToDefaultNetwork: async () => {
            try {
                await safeProviderRequest(provider, 'wallet_switchEthereumChain', [{ chainId: DEFAULT_NETWORK.chainId }], 60000 // 60 second timeout for user interaction
                );
            }
            catch (error) {
                ...
                if (error.code === 4902) {
                    try {
                        await safeProviderRequest(provider, 'wallet_addEthereumChain', [DEFAULT_NETWORK], 60000 // 60 second timeout for user interaction
                        );
                    }
                    catch {
        ...
        customRequest: async (method, params = []) => {
            try {
                ...
                const isWalletInteraction = method.includes('wallet_') ||
                ...
                const result = await safeProviderRequest(provider, method, params, timeout);
                ...
                throw new Web3ProviderError(`Custom request failed: ${method}`, ERROR_CODES.JSON_RPC_ERROR, { method, params, error });
            }
        },
        /**
         * Utility functions
